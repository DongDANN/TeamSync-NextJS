    ...
    <section className="rounded-3xl border border-black/10 bg-white">
      ...
      <div className="flex flex-col gap-4 border-b border-black/10 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-black/50">{issues.length} issue{issues.length !== 1 && 's'}</p>
        </div>
        <AnimatedButton
          type="button"
      ...
      {/* ── Search + Filters ── */}
      <div className="flex flex-wrap items-center gap-3 border-b border-black/10 px-6 py-3">
        <label className="relative flex-1 min-w-[200px] max-w-xs">
          <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-black/35">
          ...
          <input
            ...
            className="h-9 w-full rounded-lg border border-black/15 bg-white pl-9 pr-3 text-sm outline-none transition focus:border-black"
          />
          {search && (
            <button type="button" onClick={() => setSearch('')} className="absolute inset-y-0 right-0 flex items-center pr-3 text-black/35 hover:text-black">
        ...
        <div className="relative">
          <button
            ...
            className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-black/15 px-3 text-xs font-medium text-black/70 transition hover:border-black/40"
          >
            {filterType === 'all' ? 'All Types' : getIssueType(filterType)?.label ?? 'Type'}
            <FiChevronDown className="h-3 w-3" />
          </button>
          {showTypeDropdown && (
            <div className="absolute right-0 top-full z-20 mt-1 w-44 rounded-xl border border-black/15 bg-white p-1 shadow-[0_12px_30px_-12px_rgba(0,0,0,0.25)]">
              <button type="button" onClick={() => { setFilterType('all'); setShowTypeDropdown(false) }}
                className={`flex w-full items-center rounded-lg px-3 py-2 text-left text-xs font-medium transition ${filterType === 'all' ? 'bg-black text-white' : 'text-black hover:bg-black/5'}`}>
                All Types
              </button>
              {config.issueTypes.map((t) => (
                <button key={t.id} type="button" onClick={() => { setFilterType(t.id); setShowTypeDropdown(false) }}
                  className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-medium transition ${filterType === t.id ? 'bg-black text-white' : 'text-black hover:bg-black/5'}`}>
          ...
          {showPriorityDropdown && (
            <div className="absolute right-0 top-full z-20 mt-1 w-44 rounded-xl border border-black/15 bg-white p-1 shadow-[0_12px_30px_-12px_rgba(0,0,0,0.25)]">
              <button type="button" onClick={() => { setFilterPriority('all'); setShowPriorityDropdown(false) }}
                className={`flex w-full items-center rounded-lg px-3 py-2 text-left text-xs font-medium transition ${filterPriority === 'all' ? 'bg-black text-white' : 'text-black hover:bg-black/5'}`}>
                All Priorities
              </button>
              {config.priorities.map((p) => (
                <button key={p.id} type="button" onClick={() => { setFilterPriority(p.id); setShowPriorityDropdown(false) }}
                  className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-medium transition ${filterPriority === p.id ? 'bg-black text-white' : 'text-black hover:bg-black/5'}`}>
        ...
        {(filterType !== 'all' || filterPriority !== 'all' || search) && (
          <button type="button" onClick={clearFilters} className="text-xs text-black/40 underline transition hover:text-black">Clear filters</button>
        )}
      </div>
      ...
      <div className="overflow-x-auto">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="mb-3 rounded-full border border-black/10 bg-zinc-50 p-4">
              <FiSearch className="h-6 w-6 text-black/30" />
            </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-black/5 text-left text-[10px] font-medium uppercase tracking-[0.1em] text-black/40">
      ...
      {/* ── Footer ── */}
      <div className="border-t border-black/10 px-6 py-3 text-[11px] text-black/40">
        ...
        >
          ...
          >
            <button
              type="button"
              onClick={() => setPreviewUrl(null)}
              className="absolute -top-10 right-0 rounded-lg p-1.5 text-white/60 transition hover:bg-white/10 hover:text-white"
            >
