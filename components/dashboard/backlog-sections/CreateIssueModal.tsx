    ...
    <ModalFrame onClose={handleClose}>
      ...
      <div className="overflow-y-auto px-6 py-5">
        <div className="space-y-4">
          ...
          {/* Attachments */}
          <div>
            <label className="block text-[10px] font-medium uppercase tracking-[0.08em] text-black/50">Attachments</label>
            <div className="mt-1">
              <button
                type="button"
                  ...
                  {newAttachments.map((a) => (
                    <li key={a.id} className="flex items-center justify-between rounded-lg border border-black/10 bg-white px-3 py-2">
                      <div className="flex items-center gap-2 overflow-hidden min-w-0">
                        ...
                        <span className="truncate text-xs font-medium text-black">{a.fileName}</span>
                        <span className="shrink-0 text-[10px] text-black/40">{formatFileSize(a.fileSize)}</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => removeAttachment(a.id)}
                        className="rounded p-0.5 text-black/30 hover:text-red-600 shrink-0 ml-2"
                      >
      ...
      {/* Footer */}
      <div className="flex shrink-0 items-center justify-end gap-3 border-t border-black/10 px-6 py-4">
        <button
          type="button"
          onClick={handleClose}
          className="rounded-lg px-4 py-2 text-xs font-medium text-black/50 transition hover:text-black"
        >
          Cancel
        </button>
        <button
